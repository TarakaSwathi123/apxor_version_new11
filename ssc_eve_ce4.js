(function (global, factory) {
  ...
  var Frequency = /*#__PURE__*/_createClass(function Frequency() {
    ...
    _defineProperty(this, "_sessionCountNew", void 0);
    _defineProperty(this, "_dayLimit", void 0);
    _defineProperty(this, "_daysCount", void 0);
    ...
    _defineProperty(this, "parse", function () {
      ...
      try {
        ...
        if (_this._enable_session_limit) {
          ...
          _this._sessionCountNew = (_config$frequency2 = config.frequency) === null || _config$frequency2 === void 0 ? void 0 : _config$frequency2.session_count;
        }
        ...
        if (_this._enable_days_limit) {
          _this._daysCount = config.frequency.days_limit.days ? config.frequency.days_limit.days : 1;
          _this._dayLimit = config.frequency.days_limit.limit;
        }
        ...
        if (_this._validity === "SESSION") {
          ...
          if (_this._count <= 0) {
            ...
            if (Apxor.getController().isTestDevice()) {
              validationToastDisplayandRemoval("Maximum overall limit reached for campaign name ".concat(_this._meta.name));
            }
            return false;
          }
      ...
      try {
        if (_this._count <= 0) {
          if (_this._validity === "OVERALL") {
            ...
            if (Apxor.getController().isTestDevice()) {
              validationToastDisplayandRemoval("Maximum session limit reached for campaign name ".concat(_this._meta.name));
            }
            _this.logNonEligibleUserEvent("Session limit reached");
          }
          return false;
        ...
        if (_this._enable_session_limit) {
          var sessionCountFromConfig = parseInt(_this._sessionCountNew);
        ...
        if (_this._enable_repetition_gap) {
          if (_this._repetitionUnit != "session") {
            ...
            if (!isNull(isObjectWithGivenKeyPresent(repetition_times, _this._uuid))) {
              var currentObj = isObjectWithGivenKeyPresent(repetition_times, _this._uuid);
              var current_repetition_time = currentObj[_this._uuid];
              ...
              if (current_time - current_repetition_time < timeInSecFromConfig) {
                _this.logNonEligibleUserEvent("Config does not satisfy time difference              ");
                if (Apxor.getController().isTestDevice()) {
                  validationToastDisplayandRemoval("Config does not satisfy time difference for campaign name ".concat(_this._meta.name));
                }
                return false;
              }
            }
            ...
            if (!isNull(isObjectWithGivenKeyPresent(sessionsTowait, _this._uuid))) {
              var _currentObj = isObjectWithGivenKeyPresent(sessionsTowait, _this._uuid);
              var current_repetition_session = _currentObj[_this._uuid];
              ...
              if (current_session - current_repetition_session < parseInt(_this._repetitionValue)) {
                _this.logNonEligibleUserEvent("Config does not satisfy session difference              ");
                if (Apxor.getController().isTestDevice()) {
                  validationToastDisplayandRemoval("Config does not satisfy session difference for campaign name ".concat(_this._meta.name));
                }
                return false;
              }
            }
        ...
        if (_this._sessionCount !== 0 && _this.DSLimits) {
          ...
          if (sessionCountInConfig - _thisSessionCount <= 0) {
            _this.logNonEligibleUserEvent("Session limit reached");
            if (Apxor.getController().isTestDevice()) {
              validationToastDisplayandRemoval("Session limit reached for campaign name ".concat(_this._meta.name));
            }
            return false;
          }
        }
        ...
        if (_this._dayCount !== 0) {
          ...
          if (dayCountInConfig - thisDayCount <= 0) {
            ...
            if (Apxor.getController().isTestDevice()) {
              validationToastDisplayandRemoval("Day limit reached for campaign name ".concat(_this._meta.name));
            }
            return false;
          }
        }
        if (_this._daysCount > 0 && _this._enable_days_limit) {
          var daysCount = _this._daysCount;
          var shownLimit = 0;
          ...
          if (shownLimit > _this._dayLimit) {
            Logger$5.info("Shown Limit Crossed");
            if (Apxor.getController().isTestDevice()) {
              validationToastDisplayandRemoval("Days limit reached for campaign name ".concat(_this._meta.name));
            }
            return false;
          }
        }
  ...
  var Attributes = /*#__PURE__*/_createClass(function Attributes() {
    ...
    _defineProperty(this, "_compareAttributes", function (attributes, expected) {
      ...
      try {
        var _loop = function _loop() {
            var item = expected[index];
            if (!(item.name in attributes) || status === false) {
              return {
                v: false
              };
            }
  ...
  var Validity = /*#__PURE__*/_createClass(function Validity() {
    ...
    _defineProperty(this, "_excludeStartTime", -1);
    _defineProperty(this, "_excludeEndTime", -1);
    _defineProperty(this, "_is_user_timezone", void 0);
    _defineProperty(this, "_timeLimits", false);
    _defineProperty(this, "_timeExcludes", false);
      ...
      try {
        ...
        if (isDefined(config.at_specific_time)) {
          ...
          if (_this._timeLimits && isDefined(config.time_limits)) {
            _this._startTime = config.time_limits.start_time;
            _this._endTime = config.time_limits.end_time;

            // If invalid format is passed, return false
            if (isNumber(_this._startTime) || isNumber(_this._endTime)) {
        ...
        if (isDefined(config.time_excludes_in_day)) {
          _this._timeExcludes = config.time_excludes_in_day;
          if (_this._timeExcludes && isDefined(config.time_excludes)) {
            _this._excludeStartTime = config.time_excludes.start_time;
            _this._excludeEndTime = config.time_excludes.end_time;
            // If invalid format is passed, return false
            if (isNumber(_this._excludeStartTime) || isNumber(_this._excludeEndTime)) {
    ...
    _defineProperty(this, "validate", function () {
      ...
      if (currentTime > _this._startDate && currentTime < _this._endDate && _this._days_in_week.includes(currentDay)) {
        if (_this._timeLimits && !(_timeEvaluator(currentTimeIn24hourFormat, _this._startTime, "GT") && _timeEvaluator(currentTimeIn24hourFormat, _this._endTime, "LT"))) {
          _this._not_in_specified_time = true;
        }
        if (_this._timeExcludes && isTimeBetween(_this._excludeStartTime, _this._excludeEndTime, currentTimeIn24hourFormat)) {
          _this.logNonEligibleUserEvent("Exclude Time limits check failed");
          if (Apxor.getController().isTestDevice()) {
            validationToastDisplayandRemoval("Exclude Time Limits check failed for campaign name ".concat(_this._meta.name));
          }
          return false;
        }
        return !_this._timeLimits || _timeEvaluator(currentTimeIn24hourFormat, _this._startTime, "GT") && _timeEvaluator(currentTimeIn24hourFormat, _this._endTime, "LT");
        ...
        _this.logNonEligibleUserEvent("Current week day did not match with Configured Week day");
        if (Apxor.getController().isTestDevice()) {
          validationToastDisplayandRemoval("Current week day did not match with Configured Week day for campaign name ".concat(_this._meta.name));
        }
        return false;
      }
      return false;
  ...
  var ConfigItem = /*#__PURE__*/_createClass(function ConfigItem() {
    ...
    _defineProperty(this, "parse", function () {
      ...
      try {
        ...
        if (_this._meta._isExperiment) {
          var _this$_meta$_attr;
          _this._variant_code = (_this$_meta$_attr = _this._meta._attr) === null || _this$_meta$_attr === void 0 ? void 0 : _this$_meta$_attr[APX_VARIANT_CODE];
        } else {
          if (_this._meta._only_context) {
            _this._variant_code = "CG";
          } else {
            _this._variant_code = "TG";
          }
        }
    ...
    _defineProperty(this, "_validateAllConditions", function () {
      ...
      if (!((_window$ApxorRTM = window.ApxorRTM) !== null && _window$ApxorRTM !== void 0 && _window$ApxorRTM.isBadgePresent && (_window$ApxorRTM2 = window.ApxorRTM) !== null && _window$ApxorRTM2 !== void 0 && _window$ApxorRTM2.badgesLists.includes(_this._uuid) && Apxor.getController().isBadgeTriggerSatisfied(_this._uuid))) {
        ...
        if (_this._meta._type === "SURVEY") {
          eventProps.apx_survey_name = _this._meta._name;
          eventProps.apx_survey_id = _this._uuid;
        }
        if (_this._meta._type != "SURVEY") {
          Apxor === null || Apxor === void 0 || Apxor.logEvent("apx_trigger_satisfied", eventProps);
        }
      }
      ...
      if (isSatisfied) {
        ...
        if (!((_window$ApxorRTM3 = window.ApxorRTM) !== null && _window$ApxorRTM3 !== void 0 && _window$ApxorRTM3.isBadgePresent && (_window$ApxorRTM4 = window.ApxorRTM) !== null && _window$ApxorRTM4 !== void 0 && _window$ApxorRTM4.badgesLists.includes(_this._uuid) && Apxor.getController().isBadgeTriggerSatisfied(_this._uuid))) {
          ...
          if (_this._meta._type === "SURVEY") {
            _eventProps.apx_survey_name = _this._meta._name;
            _eventProps.apx_survey_id = _this._uuid;
          }
          Apxor === null || Apxor === void 0 || Apxor.logEvent("apx_trigger_satisfied", _eventProps, false, "", _apx_nudge_type);
        }
        if (_this._across_sessions) {
          localStorage.removeItem("apx_nokpi_triggerdetails");
        }
        ...
        var _eventProps2 = _objectSpread2(_objectSpread2({}, _this._meta._attr), {}, {
          message_name: _this._meta._name,
          id: _this._uuid
        });
        var _apx_nudge_type2 = _this._meta._type === "SURVEY" ? "survey" : "campaign";
        if (_this._meta._type === "SURVEY") {
          _eventProps2.apx_survey_name = _this._meta._name;
          _eventProps2.apx_survey_id = _this._uuid;
        }
        if (_this._meta._only_context === true) {
          Apxor.logEvent(APX_CONTEXT_EVALUATED, _eventProps2, false, "", _apx_nudge_type2);
        } else {
          Apxor.logEvent(APX_CONTEXT_EVALUATED, _eventProps2, false, "", _apx_nudge_type2);
        }
        // Emit event
    ...
    _defineProperty(this, "_validateAllGoalEvents", function () {
      ...
      if (isSatisfied) {
        ...
        if (_this._meta._only_context === true) {
          var apx_nudge_type = _this._meta._type === "SURVEY" ? "survey" : "campaign";
          var eventProps = _objectSpread2(_objectSpread2({}, _this._meta._attr), {}, {
            message_name: _this._meta._name,
            id: _this._uuid
          });
          if (_this._meta._type === "SURVEY") {
          ...
          Apxor.logEvent(APX_CONTEXT_EVALUATED, eventProps, false, "apxor", apx_nudge_type);
        }
        // Emit event
        Apxor.getController().dispatchEvent(_this._meta._type, {
          name: _this._meta._type,
